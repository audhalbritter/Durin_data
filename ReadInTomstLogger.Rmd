...
micro_data3 <- microclimate2 %>%
  ...
  group_by(loggerID, daytime,dayornight,species,drt_treatment,fire_class,plotID) %>%
=======
  filter(cutting != "cut" & datetime > "2023-04-28" & datetime < "2023-06-20" & land_type == "DroughtNet" & loggerID != 95221050) %>%
  mutate(daytime = floor_date(datetime, "day") ) %>%
  group_by(loggerID, daytime,species,drt_treatment,fire_class) %>%
>>>>>>> beffb79ee64e525a1a801e208a8ab7acf7906cdd
  summarise(
    air_temperature_mean = mean(value[sensor == "air_temperature"]),
    air_temperature_min = min(value[sensor == "air_temperature"]),
...
model_1_air <- lmer(air_temperature_mean ~ site + (1|site/land_type/loggerID), data = micro_linear)

summary(model_1_air)
#fixed
model_2_air <- lmer(air_temperature_mean ~ site + land_type + (1|site/land_type/individual), data = micro_linear)

summary(model_2_air)

model_3_air <- lmer(air_temperature_mean ~ site*land_type + (1|site/land_type/individual), data = micro_linear)

summary(model_3_air)

model_4_air <- lmer(air_temperature_mean ~ site*land_type*species + (1|site/land_type/individual), data = micro_linear)

summary(model_4_air)
anova(model_1_air, model_2_air, model_3_air, model_4_air)
#random
...
###ground_temperature
# Example of fixed effects comparisons, with same random effects structure
model_1_ground <- lmer(ground_temperature_mean ~ site + (1|site/land_type/individual), data = micro_linear)
...
###soil_temperature
# Example of fixed effects comparisons, with same random effects structure
model_1_soil <- lmer(soil_temperature_mean ~ site + (1|site/land_type/individual), data = micro_linear)
...
###air_temperature
# Example of fixed effects comparisons, with same random effects structure
model_1_moist <- lmer(soil_moisture_mean ~ site + (1|site/land_type/individual), data = micro_linear)
...
```
```{r linear_lygra_dn}
micro_linear2 <- micro_data3

micro_linear2$individual <- with(micro_linear2, interaction(plotID, species))

###air_temperature
# Example of fixed effects comparisons, with same random effects structure
dn_model_1_air <- lmer(air_temperature_mean ~ drt_treatment + (1|plotID/loggerID), data = micro_linear2)
summary(dn_model_1_air)

dn_model_2_air <- lmer(air_temperature_mean ~ drt_treatment + (1|plotID/loggerID) + (1|fire_class), data = micro_linear2)
summary(dn_model_2_air)

dn_model_3_air <- lmer(air_temperature_mean ~ drt_treatment + (1|plotID/loggerID) + (1|species), data = micro_linear2)
summary(dn_model_3_air)

dn_model_4_air <- lmer(air_temperature_mean ~ dayornight + (1|plotID/loggerID), data = micro_linear2)
summary(dn_model_4_air)

stargazer(dn_model_4_air, type = "text",
          digits = 3,
          star.cutoffs = c(0.05, 0.01, 0.001),
          digit.separator = "")

dn_model_5_air <- lmer(air_temperature_mean ~ species + (1|plotID/loggerID), data = micro_linear2)
summary(dn_model_5_air)

dn_model_6_air <- lmer(air_temperature_mean ~ fire_class + (1|plotID/loggerID) + (1|species2), data = micro_linear2)
summary(dn_model_6_air)
#fixed
