...
```{r box_plot_tryout}
test_data <- microclimate %>%
  filter(cutting != "cut" & datetime > "2023-06-24" & datetime < "2023-07-19" & date_in < "07/01/2023") %>%
  mutate(daytime = floor_date(datetime, "day") ) %>%
  group_by(loggerID, daytime, site, land_type ) %>%
  summarise(
...
test_2 <- test_data %>%
   group_by(site) |>
  summarise(meanTemp=mean(air_temperature_mean), minT = min(air_temperature_min),maxT = max(air_temperature_max))

base <- ggplot(test_2, aes(x = site, y = meanTemp, fill = site))
graph3 <- base + 
  geom_bar(stat = "identity", position = "dodge") +
  labs(y = "Temperature (°C)") +
  theme_bw() +
  scale_x_discrete(limits = c("Senja", "Kautekaino", "Lygra")) +
  geom_errorbar(aes(ymin = minT, ymax = maxT), 
                position = position_dodge(width = 0.9), width = 0.15)

print(graph3)

base <- ggplot(test_data, aes(x = site, y = air_temperature_mean))
graph3bis <- base + 
  geom_boxplot(outlier.shape = NA) +
  labs(y = "Temperature (°C)") +
  theme_bw() +
  scale_x_discrete(limits = c("Senja", "Lygra", "Kautekaino")) +
