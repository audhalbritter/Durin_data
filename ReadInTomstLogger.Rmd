...
base_air <- ggplot(micro_data3, aes(x = dayornight, y = air_temperature_mean, fill = dayornight))
graph_air <- base_air + 
  geom_boxplot(outlier.shape = NA) +
  labs(y = "Air Temperature +15cm (°C)",x="Type of the land") +
  theme_bw() +
  scale_x_discrete(labels = c("day" = "day", "night" = "night")) +
  facet_wrap(~drt_treatment, ncol = 3, scales = "free_x") +
  scale_fill_manual(values = c("day" = "#1e90ff", "night" = "#ff0800"))

ggsave(file.path("data/plots/testdaynight", "By_day_and_night_air.png"), graph_air, height = 40, width = 80, units = "cm")
```
